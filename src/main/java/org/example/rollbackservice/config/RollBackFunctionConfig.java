                ...
                .flatMap(request ->
                        portOneClient.cancelPayment(request.impUid(), request.paidAmount())
                                ...
                                .flatMap(rsp -> {
                                    ...
                                    if (res == null) {
                                        String reason = String.format("포트원에서 %s", rsp.message());
                                        throw new IllegalStateException(reason);
                                    }
                                    log.info("결제 롤백 완료: impUid={}, amount={}", res.imp_uid(), res.amount());
                                    return lectureClient.rollbackDiscount(request.productId())
                                            .thenReturn(new RollBackResponseMessage(
                                                    request.purchaseId(), true, "결제 롤백 완료 + 할인권 재고 복구 완료"
                                            ))
                                            .onErrorResume(e -> {
                                                ...
                                                if (e instanceof WebClientResponseException.NotFound || e instanceof WebClientResponseException.BadRequest) {
                                                    log.warn("productId={}는 복구 대상이 아닙니다. DB에 저장하지 않겠습니다.", request.productId());
                                                    return Mono.just(new RollBackResponseMessage(request.purchaseId(), true,
                                                            "할인권 재고 복구 거부: " + e.getMessage()));
                                                }

                                                String reason = "할인권 재고 복구 실패: " + e.getMessage();
